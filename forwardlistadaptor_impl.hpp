...
template <typename Compare, std::size_t N>
ack ForwardListAdaptor<Compare, N>::insert_(const Message::order& o) {
...
ForwardListAdaptor<Compare, N>::execute_(const Message::order& exec_order) {
  ...
  while (next != orders_.end() && remaining > 0 &&
    ...
    if (remaining >= next->size_) {
      ...
      trades.emplace_back(next->seqNum_, exec_order.seqNum_, next->orderId_, standingOrderSide,
                          next->price_, next->size_);
      // Liquidity-taking order
      trades.emplace_back(exec_order.seqNum_, next->seqNum_, exec_order.orderId_, exec_order.side_,
                          next->price_, next->size_);
...
template <typename Compare, std::size_t N>
ack ForwardListAdaptor<Compare, N>::remove_(const Message::order& o) {
  auto it = orders_.before_begin();
  auto next = orders_.begin();
...
template <typename Compare, std::size_t N>
ack ForwardListAdaptor<Compare, N>::update_(const Message::order& o) {
  remove_(o);
  return insert_(o);
}

template <typename Compare, std::size_t N>
std::optional<long> ForwardListAdaptor<Compare, N>::getBBOPrice() const {
  if (!orders_.empty())
...
template <typename Compare, std::size_t N>
std::optional<long> ForwardListAdaptor<Compare, N>::getBBOSize() const {
  if (!orders_.empty())
...
template <typename Compare, std::size_t N>
std::optional<unsigned> ForwardListAdaptor<Compare, N>::sizeAtPrice(long price) const {
  if (!orders_.empty()) {
    unsigned qty = 0;
...
template <typename Compare, std::size_t N>
std::vector<Book::PriceLevel> ForwardListAdaptor<Compare, N>::getBook() const {
