...
template <typename T>
class BookFeeder {
  enum class STATES { START = 0, UPDATING = 1, UPDATED = 2, ERROR = 3, NUMSTATES };
...
class BookSide : public BookFeeder<BookSide<Container>> {
 public:
  ...
  BookSide(const BookSide& rhs) : side_{rhs.side_} {}
  BookSide(BookSide&& rhs) : side_{std::move(rhs.side)} {}
  BookSide(const Container& rhs) : side_{rhs} {}

  BookSide(Container&& rhs) : side_{std::move(rhs.side_)} {}
...
class OrderBook {
 public:
  ...
  AckTrades processOrder(const Message::order& order);
 ...
 private:
  ...
  bool _crossedBook(const Message::eventLOBSTER&);
  bool _undersizedCross(const Message::eventLOBSTER&);
