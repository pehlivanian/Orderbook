...
class LockFreeOrderedMessageQueue {
 public:
  LockFreeOrderedMessageQueue() : buffer_{BUFFER_SIZE} {}
  ...
  bool enqueue(EventType&& e) {
    ...
    int expected = -1;
    if (slot.seqNum_.compare_exchange_strong(expected, sequenceNumber, std::memory_order_release,
...
class Spinlock {
 public:
  ...
  void unlock() { p_.store(saved_p_, std::memory_order_release); }
...
class Consumer {
  ...
  struct worker {
    ...
    worker(Consumer<EventType, OrderType>* outer, int id) : outer_{outer}, id_{id} {}
 ...
 public:
  ...
  Consumer(std::shared_ptr<SPMCInnerQ> q_source, std::shared_ptr<OrderedMPMCq> q_target,
           std::size_t num_workers, std::chrono::milliseconds millis = 0ms)
      : SPMCqueue_source_{q_source},
        MPMCqueue_target_{q_target},
