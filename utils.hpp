...
struct order {
  ...
  bool operator==(const Message::order& rhs) {
    return (side_ == rhs.side_) &&
           (fabs(price_ - rhs.price_) < std::numeric_limits<double>::epsilon());
  }
};
...
Message::order eventLOBSTERToOrder(const Message::eventLOBSTER& e) {
  return Message::order{e.seqNum_, e.direction_, e.time_,     e.orderId_,
                        e.price_,  e.size_,      e.eventType_};
}
...
std::ostream& operator<<(std::ostream& os, const Message::order& o) {
  os << "{ " << o.seqNum_ << ", " << o.time_ << ", " << o.side_ << ", " << o.price_ << ", "
     << o.size_ << "}";
  return os;
}
...
std::osyncstream& operator<<(std::osyncstream& os, const Message::order& o) {
  os << "{ " << o.seqNum_ << ", " << o.time_ << ", " << o.side_ << ", " << o.price_ << ", "
     << o.size_ << ", " << o.orderType_ << "}";
  return os;
}

std::ostream& operator<<(std::ostream& os, const Message::event& e) {
  os << "{ " << e.seqNum << ", " << e.msgType << ", " << e.side << ", " << e.level << ", "
     << e.price << ", " << e.size << "}";
  return os;
}

std::ostream& operator<<(std::ostream& os, const Message::eventLOBSTER& e) {
  os << "{" << std::setprecision(14) << e.seqNum_ << ", " << e.time_ << ", " << e.eventType_ << ", "
     << e.orderId_ << ", " << e.size_ << ", " << e.price_ << ", " << e.direction_ << "}";
  return os;
}
...
std::vector<Message::event> readDataDefault(const boost::filesystem::path& path) {
  ...
  */
  ...
  while (std::getline(fh, line)) {
    auto tokens = split(line, ',');
    Message::event e = {std::stoul(tokens[0]),
                        tokens[1][0],
                        tokens[2][0],
                        std::stoi(tokens[3]),
                        static_cast<long>(std::stod(tokens[4]) * MULT),
                        std::stoul(tokens[5])};
    res.push_back(e);
  }
